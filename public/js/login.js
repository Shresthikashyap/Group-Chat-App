...
const save = async(event) =>{
    try{
        ...
        if(groupId){
            response = await axios.post(`http://13.127.51.17:3000/user/login?groupId=${groupId}`,loginDetails);
        }
        else{
            response = await axios.post("http://13.127.51.17:3000/user/login",loginDetails);
        }
