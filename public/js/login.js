...
const save = async(event) =>{
    try{
        ...
        if(groupId){
            response = await axios.post(`http://3.27.43.97:3000/user/login?groupId=${groupId}`,loginDetails);
        }else{
            response = await axios.post("http://3.27.43.97:3000/user/login",loginDetails);
        }
