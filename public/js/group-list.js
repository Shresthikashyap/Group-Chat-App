...
window.addEventListener("DOMContentLoaded",async()=>{
    try{
        ...
        const userId = decodedToken.id;
            ...
            const groups = await axios.get(`http://13.51.156.137/group/group-list/${userId}`,{headers:{Authorization:token}}) 
            ...
            groups.data.list.forEach((group) => {
                    ...
                    groupDiv.addEventListener('click', async() => {
                      ...
                      const admin = await axios.get(`http://13.51.156.137/admin/checkadmin/${userId}/${groupId}`,{headers:{Authorization:token}})
                      
                      if(admin.data.admin.isAdmin === true) 
                      { localStorage.setItem('link',`http://13.51.156.137/signup.html?groupId=${groupId}`)}
