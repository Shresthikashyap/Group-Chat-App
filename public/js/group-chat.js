...
const getMessages = async (lastMsgId,groupId) => {
 try {
    ...
    const response = await axios.get(`http://54.234.45.200:3000/message/get-message/${lastMsgId}/${groupId}`,
...
const send = async(event) => {
    try{
         ...
         const response = await axios.post(`http://54.234.45.200:3000/message/post-message/${groupId}`,msgDetails,
         {headers:{'Authorization':token}});
...
fileInput.addEventListener('input', handleSelectedFile = async(event) => {
  try{
      ...
      const token = localStorage.getItem('token');
      const fileStored = await axios.post(`http://54.234.45.200:3000/file/filestored/${groupId}`,formData,
