...
window.addEventListener("DOMContentLoaded",async()=>{
              ...
              adminId = checkAdmin.data.admin.userId;
                ...
                if(checkAdmin.data.admin.isAdmin === true){
                 ...
                 group.append(deleteGroup);              
                    ...
                    const memberList = await axios.get(`http://23.20.253.19:3000/group/memberlist/${groupId}`,{
                    ...
                    const members = document.getElementById('memberlist');
                      ...
                      memberList.data.list.forEach(async(member) => {         /*memberlist the loop */
                        ...
                        const admin = await axios.get(`http://23.20.253.19:3000/admin/checkadmin/${member.id}/${groupId}`,{headers:{Authorization:token}})
    ...
    const removeMember = async(userId,groupId) =>{
        try{
            ...
            const token = localStorage.getItem('token'); 
            const removedUser = await axios.get(`http://23.20.253.19:3000/admin/removeuser/${userId}/${groupId}`,
            {headers:{Authorization:token}});
    ...
    const newAdmin = async(memberId, _groupId) => {
        try{
          ...
          const token = localStorage.getItem('token')
          const adminDetails = await axios.get(`http://23.20.253.19:3000/admin/makeadmin/${memberId}/${_groupId}`,
          {headers:{Authorization:token}});
    ...
    const removeAdmin = async(memberId, _groupId) => {
      try{
        console.log(memberId,_groupId)
        const token = localStorage.getItem('token')
        const adminDetails = await axios.get(`http://23.20.253.19:3000/admin/removeadmin/${memberId}/${_groupId}`,
        {headers:{Authorization:token}});
  ...
  const exitGroup = async(userId,groupId) =>{
    try{
      const token = localStorage.getItem('token');
      const group = await axios.get(`http://23.20.253.19:3000/group/exitgroup/${userId}/${groupId}`,{
        headers:{Authorization:token}});
  ...
  //delete the group
    const deleteTheGroup = async(groupId) => {
      try{
          console.log(groupId);
          const token = localStorage.getItem('token'); 
          const group = await axios.get(`http://23.20.253.19:3000/admin/deletegroup/${groupId}`,{
            headers:{Authorization:token}});
  ...
  const getStoredFiles = async(groupId) => {
      try{
          const token = localStorage.getItem('token'); 
          const storedFiles = await axios.get(`http://23.20.253.19:3000/file/getfiles/${groupId}`,
          {headers:{Authorization:token}});
